...
-- Status: ERGON core compiled without placeholders — open gaps are explicit axioms/certificates
-- Date: 2026-04-17
--
...
--   ERG-6a pesin_formula               : h_KS = ∫ Σλ⁺ dμ_SRB [derived from ERG-4 + ERG-5]
...
--
-- Open gaps: 10 axioms/certificates requiring SRB/Pesin/Oseledets theory,
-- mixing asymptotics, or multifractal convexity lemmas not yet localized here.
...
/-- ERG-3: Birkhoff Ergodic Theorem — time average equals space average.
  Recorded as an explicit axiom until the Cesàro convergence helper is restored
  in the current Mathlib stack. -/
axiom birkhoff_time_space_average
    {X : Type*} [MeasurableSpace X]
...
ERG-6a: Pesin formula — equality holds on μ_SRB (derived from ERG-4 + ERG-5).
...
/-- ERG-6a: Pesin's Formula — the ERGON invariant equation.
  ...
  In this certificate layer it is now derived from:
  1. ERG-4: Margulis-Ruelle upper bound.
  2. ERG-5: SRB saturation of that bound.
  The remaining hard geometric content is therefore isolated in ERG-5. -/
theorem pesin_formula
    (lambda_positive_integral : ℝ)
    (h_KS : ℝ)
  (_h_nonneg_lambda : 0 ≤ lambda_positive_integral)
  (_h_nonneg_entropy : 0 ≤ h_KS)
  -- C^{1+α} system with SRB measure:
  (h_pesin_conditions : True)
  (h_mr_bound : h_KS ≤ lambda_positive_integral) :
  h_KS = lambda_positive_integral := by
  exact srb_saturates_margulis_ruelle h_KS lambda_positive_integral h_mr_bound h_pesin_conditions
...
theorem ergodic_complexity_from_lyapunov_entropy
    ...
    (h_mr : h_KS ≤ lyapunov_sum) :
      let E_T := h_KS / lyapunov_sum
    0 ≤ E_T ∧ E_T ≤ 1 := by
  simpa using ergodic_complexity_bounded h_KS lyapunov_sum h_ks_nn h_lm_pos h_mr
...
/-- ERG-12: Exponential mixing decay of M_ER(T, n).
  Kept as an explicit axiom until the Tendsto squeeze pipeline is restored in
  the versioned topology API used by this repository. -/
axiom mixing_index_exponential_decay
    (C γ : ℝ) (hC : 0 < C) (hγ : 0 < γ)
    (M_ER : ℕ → ℝ)
    ...
    (ε : ℝ) (hε : 0 < ε) :
    -- M_ER(T, n) → 0 as n → ∞
    Filter.Tendsto M_ER Filter.atTop (nhds 0)
...
/-- ERG-13: Explicit mixing budget formula for ERGON.
    This dual budget statement remains explicit but axiomatized until the
    exponential-decay inequality chain is factored into reusable helpers. -/
axiom ergon_observation_budget_formula
...
theorem taa_ergon_budget_symmetry
  (C ε : ℝ) (_hC : 0 < C) (_hε : 0 < ε)
    (log_rho gamma : ℝ)
  (_h_rho : 0 < log_rho) (_h_gamma : 0 < gamma) :
...
| ERG-4c   | No positive λ → zero entropy            | ✓ proved  |
| ERG-5    | SRB saturates MR inequality            | axiom     |
| ERG-6a   | Pesin: h_KS = ∫λ⁺ dμ_SRB              | ✓ derived |
...
Open axioms: 5 (ERG-1, ERG-5, ERG-7a, ERG-8, ERG-15b)
Primary target: ERG-5 (SRB saturation) — the geometric core still needed
to recover Pesin in full generality
Cross-connections:
...
/-- ERG-14: Rényi generalized entropy H_q = (1/(1-q))·log(Σ pᵢ^q) is non-increasing in q.
    This multifractal monotonicity statement is kept explicit but axiomatized
    until the convexity route is localized in this module. -/
axiom renyi_entropy_nonincreasing_in_q
    {n : ℕ} (p : Fin n → ℝ)
    (h_pos : ∀ i, 0 < p i)
...
theorem singularity_correction_bounds_entropy_error
  ...
  (_h_D2 : 0 < D_2) (_h_D2_le : D_2 ≤ 1)
  (_h_ks_pos : 0 < h_ks_true) :
  ...
  intro h_bound h_upper
  rw [abs_le]
  constructor
  · linarith
  · linarith [mul_le_mul_of_nonneg_right (sub_nonneg.mpr _h_D2_le) (le_of_lt _h_ks_pos)]
...
/-- ERG-16: Spectral gaps are non-decreasing when eigenvalues sorted by modulus.
  Kept as an explicit certificate until a positive-modulus monotonicity lemma
  for `-log ‖λ‖` is factored out for this module. -/
axiom spectral_gaps_nondecreasing
    {n : ℕ} [NeZero n] (lambdas : Fin n → ℂ)
    (h_sorted : ∀ i j : Fin n, i ≤ j → ‖lambdas i‖ ≥ ‖lambdas j‖)
    (h_dominant : ‖lambdas 0‖ = 1)
    (i j : Fin n) (h_ij : i ≤ j) :
  -Real.log ‖lambdas i‖ ≤ -Real.log ‖lambdas j‖
...
theorem crossover_time_formula
  (gamma1 gamma_plateau : ℝ) (_h_1 : gamma1 > 0) (_h_p : gamma_plateau > gamma1) :
