...
for filename in files_to_update:
    ...
    # Avoid regex, use plain string replace
    old_str = r"There exists a constructive morphism \Phi_AC such that \Phi_AC(f) is represented as a finite composition of FMA/GEMM operations."
    new_str = r"There exists a constructive morphism \Phi_{AC}, formally tracked by `verify_urt_koopman_bound` at runtime, which bounds the computable domain mapped to FMA/GEMM operations. Non-linear Koopman reductions empirically demonstrate mathematically bounded truncation errors \delta(d)."
    content = content.replace(old_str, new_str)
    
    if "## Formal Verification Reality" not in content:
        content += """
